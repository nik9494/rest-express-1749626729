...
interface UseHeroGameOptions {
  roomId?: string;
  gameId?: string;
  userId?: string;
}

export const useHeroGame = ({ roomId, gameId, userId }: UseHeroGameOptions = {}) => {
  ...
  // Handle tap (только для игроков, не для наблюдателей)
  const handleTap = useCallback(() => {
    if (!isStarted || isFinished || !roomId || !userId || !connected || countdown !== null || isObserver) return;
  ...
  }, [isStarted, isFinished, roomId, userId, connected, triggerHapticFeedback, countdown, isObserver]);

  // Send taps to server (throttled to prevent spamming)
  const sendTapToServer = useCallback(
    throttle(() => {
      if (buffer > 0 && roomId && userId && connected && !isObserver) {
    ...
    [buffer, roomId, userId, connected, sendTap, isObserver],
  );
  ...
  // Send buffered taps
  useEffect(() => {
    if (buffer > 0 && !isObserver) {
      sendTapToServer();
    }
  }, [buffer, sendTapToServer, isObserver]);
  ...
  useEffect(() => {
    ...
    const unsubscribeGameStart = subscribe(
      ...
      (message: WebSocketMessage) => {
        if (message.room_id === roomId) {
          console.log("[HeroGame] Game starting, beginning countdown");
          setIsStarted(true);
